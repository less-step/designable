...
import { TreeNode, createResource } from '@kep-platform/designable-core'
import { useTreeNode, TreeNodeWidget, DroppableWidget, useNodeIdProps, DnFC } from '@kep-platform/designable-react'
				...
				<ArrayBase.Item index={0} record={null}>
					...
					>
						<div {...createNodeId(designer, ensureObjectItemsNode(node).id)}>
							{children.length ? children.map((node) => <TreeNodeWidget key={node.id} node={node} />) : <DroppableWidget hasChildren={false} />}
						</div>
					</Card>
				</ArrayBase.Item>
				{additions.map((node) => (
