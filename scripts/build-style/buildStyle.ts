...
const importCssCompilerToCss = async ({
	...
	styleFileContent = styleFileContent.replace(new RegExp(`\.\/${styleEntry}`), './css.css')

	return outputFile(outputFileName, transform?.(styleFileContent) || styleFileContent)
}

const getPaths = (filename: string, moduleType: 'esm' | 'lib') => {
	const styleFilePath = filename.replace(/src\//, `${moduleType}/`).replace(/\.ts$/, '.js')
...
const buildCss = async ({ filename, esmPaths, libPaths, styleEntry }: Pick<BuildStyleOptions, 'filename' | 'styleEntry'> & Record<'esmPaths' | 'libPaths', ReturnType<typeof getPaths>>) => {
	...
	return Promise.all([copy(esmPaths.cssFilePath, libPaths.cssFilePath), existsSync(esmPaths.cssSourceMapFilePath) && copy(esmPaths.cssSourceMapFilePath, libPaths.cssSourceMapFilePath)])
}
