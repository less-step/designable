...
export const IconWidget: React.FC<IIconWidgetProps> & {
	...
	const renderTooltips = (children: React.ReactElement): React.ReactElement => {
		...
		if (tooltip) {
			const title = React.isValidElement(tooltip) || isStr(tooltip) ? tooltip : tooltip.title
			const props = React.isValidElement(tooltip) || isStr(tooltip) ? {} : isObj(tooltip) ? tooltip : {}
			return (
