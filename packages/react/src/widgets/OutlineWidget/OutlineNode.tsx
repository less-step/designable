...
export const OutlineTreeNode: React.FC<IOutlineTreeNodeProps> = observer(({ node, className, style, workspaceId }) => {
	...
	return (
		<div style={style} ref={ref} className={cls(prefix, className, 'expanded')} data-designer-outline-node-id={node.id}>
			<div className={prefix + '-header'}>
				...
				<div className={prefix + '-header-content'}>
					...
					<div className={prefix + '-header-actions'} data-click-stop-propagation>
						{renderActions(node)}
						{node !== node.root && (
							<IconWidget
			...
			<div className={prefix + '-children'}>
				{node.children?.map((child) => {
					return <OutlineTreeNode node={child} key={child.id} workspaceId={workspaceId} />
				})}
			</div>
		</div>
	)
