...
export const EmptyWidget: React.FC<IEmptyWidgetProps> = observer((props) => {
		...
		return (
			<div style={{ display: 'flex', flexDirection: 'column' }}>
				<div className="animations">
					<IconWidget infer={props.dragTipsDirection === 'left' ? 'DragLeftSourceAnimation' : 'DragRightSourceAnimation'} size={240} />
					<IconWidget infer="BatchDragAnimation" size={240} />
				</div>
				<div className="hotkeys-list">
					<div>
						Selection <IconWidget infer="Command" /> + Click / <IconWidget infer="Shift" /> + Click / <IconWidget infer="Command" /> + A
					</div>
					<div>
	...
	if (!tree?.children?.length) {
		return <div className={prefix}>{props.children ? props.children : renderEmpty()}</div>
	}
	return null
})
