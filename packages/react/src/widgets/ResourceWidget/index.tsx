...
export const ResourceWidget: React.FC<IResourceWidgetProps> = observer((props) => {
	...
	const renderNode = (source: IResource) => {
		...
		return (
			<div className={prefix + '-item'} style={{ gridColumnStart: `span ${span || 1}` }} key={node.id} data-designer-source-id={node.id}>
				{thumb && <img className={prefix + '-item-thumb'} src={thumb} />}
				{icon && React.isValidElement(icon) ? <>{icon}</> : <IconWidget className={prefix + '-item-icon'} infer={icon} style={{ width: 150, height: 40 }} />}
				<span className={prefix + '-item-text'}>{<TextWidget>{title || node.children[0]?.getMessage('title')}</TextWidget>}</span>
			</div>
		)
	}
		...
		>
			...
			<div className={prefix + '-content-wrapper'}>
				<div className={prefix + '-content'}>
					...
					{remainItems ? <div className={prefix + '-item-remain'} style={{ gridColumnStart: `span ${3 - remainItems}` }}></div> : null}
				</div>
			</div>
		</div>
	)
