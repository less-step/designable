...
import { usePrefix } from '@kep-platform/designable-react'
...
export const BorderStyleSetter: React.FC<IBorderStyleSetterProps> = observer(({ className, style }) => {
						...
						{Positions.map((position, key) => {
							return (
								<Fragment key={key}>
									...
									/>
									<Field name={createBorderProp(position, 'color')} basePath={field.address.parent()} reactions={createReaction(position)} component={[ColorInput]} />
								</Fragment>
							)
						})}
					</div>
