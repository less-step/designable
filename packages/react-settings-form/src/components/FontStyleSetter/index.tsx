...
export const FontStyleSetter: React.FC<IFontStyleSetterProps> = observer((props) => {
	...
	return (
		<FoldItem label={field.title} className={cls(prefix, props.className)} style={props.style}>
			<FoldItem.Base>
				<Field name="fontFamily" basePath={field.address.parent()} component={[Select, { style: { width: '100%' }, placeholder: 'Helvetica Neue' }]} dataSource={FontFamilyOptions} />
			</FoldItem.Base>
			<FoldItem.Extra>
				<InputItems>
					<InputItems.Item icon="FontWeight" width="50%">
						<Field name="fontWeight" basePath={field.address.parent()} component={[NumberPicker, { placeholder: '400' }]} />
					</InputItems.Item>
					...
					<InputItems.Item icon="FontSize" width="50%">
						<Field name="fontSize" basePath={field.address.parent()} component={[SizeInput, { exclude: ['auto'] }]} />
					</InputItems.Item>
					<InputItems.Item icon="LineHeight" width="50%">
						<Field name="lineHeight" basePath={field.address.parent()} component={[SizeInput, { exclude: ['auto'] }]} />
					</InputItems.Item>
